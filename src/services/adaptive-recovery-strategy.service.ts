...
class AdaptiveRecoveryStrategyService {
    ...
    public calculateRecoveryStrategy(
        ...
        // Check for aggressive mode from stake manager
        let isAggressiveMode = false;
        try {
            const { stakeManager } = require('@/services/stake-manager.service');
            isAggressiveMode = stakeManager.getAggressiveMode();
        } catch (error) {
            // Fallback if stake manager is not available
            isAggressiveMode = false;
        }

        if (isAggressiveMode) {
            // Aggressive mode: recovery prediction same as start prediction
            predictionBeforeLoss = barrier;
            predictionAfterLoss = barrier; // Same as start prediction
            reasoning = `${signalType}${barrier}: Aggressive mode - both start and recovery use ${barrier} (consistent strategy)`;
            console.log(`🔥 Aggressive Mode: ${signalType}${barrier} → ${signalType}${barrier} recovery strategy applied`);
        } else if (strategy === 'CONSERVATIVE_TO_AGGRESSIVE') {
            ...
            if (signalType === 'OVER') {
                ...
                if (isBarrierAdjusted) {
                    ...
                    // Custom recovery logic: OVER 2,3,4 all recover with OVER 4
                    if (barrier >= 2 && barrier <= 4) {
                        predictionAfterLoss = 4;
                        reasoning = `OVER${barrier}: Start at barrier ${barrier}, recover with OVER 4 (unified recovery strategy)`;
                        console.log(`🎯 Unified Recovery: OVER${barrier} → OVER4 recovery strategy applied`);
                    } else {
                ...
                if (isBarrierAdjusted) {
                    ...
                    // Custom recovery logic: UNDER 7,6,5 all recover with UNDER 5
                    if (barrier >= 5 && barrier <= 7) {
                        predictionAfterLoss = 5;
                        reasoning = `UNDER${barrier}: Start at barrier ${barrier}, recover with UNDER 5 (unified recovery strategy)`;
                        console.log(`🎯 Unified Recovery: UNDER${barrier} → UNDER5 recovery strategy applied`);
                    } else {
            ...
            if (signalType === 'OVER') {
                if (isBarrierAdjusted) {
                    ...
                    if (barrier >= 2 && barrier <= 4) {
                        ...
                        predictionAfterLoss = 4; // Unified recovery with OVER 4
                        reasoning = `OVER${barrier}: Start aggressive at ${predictionBeforeLoss}, recover with OVER 4 (unified recovery strategy)`;
                    } else {
                        predictionBeforeLoss = Math.min(barrier + 1, 9); // Aggressive: one step higher
                ...
                if (isBarrierAdjusted) {
                    ...
                    if (barrier >= 5 && barrier <= 7) {
                        ...
                        predictionAfterLoss = 5; // Unified recovery with UNDER 5
                        reasoning = `UNDER${barrier}: Start aggressive at ${predictionBeforeLoss}, recover with UNDER 5 (unified recovery strategy)`;
                    } else {
                        predictionBeforeLoss = Math.max(barrier - 1, 0); // Aggressive: one step lower
                        predictionAfterLoss = barrier; // Conservative: back to barrier
